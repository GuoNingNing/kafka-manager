...
trait OffsetCache extends Logging {
  ...
  protected def readHbaseManagedConsumerOffsetByTopicPartition(consumer: String
                                                               , topic: String
                                                               , tpi: Map[Int, TopicPartitionIdentity]): Map[Int, Long] = {

    HbaseOffsetsManager.getConsumerOffset(consumer: String, topic: String)
  ...
  protected def getHbaseManagedConsumerList: IndexedSeq[ConsumerNameAndType] = {
    val consumers = HbaseOffsetsManager.getConsumers.map(name => ConsumerNameAndType(name, HbaseManagedConsumer))
    consumers.toIndexedSeq
    ...
    val consumerTopics: Set[String] = getKafkaVersion match {
      ...
      case _ =>
        consumerType match {
          ...
          case HbaseManagedConsumer =>
            getHbaseManagedConsumerTopics(consumer)
        }
    }
    ...
    val (partitionOffsets, partitionOwners) = consumerType match {
      ...
      case HbaseManagedConsumer =>
        val partitionOffsets = for {
          ...
          readHbaseManagedConsumerOffsetByTopicPartition(consumer, topic, tpi)
        }
        val partitionOwners = for {
          td <- optTopic
          tpi <- optTpi
  ...
  final def getConsumerList: ConsumerList = {
    ConsumerList(getKafkaManagedConsumerList ++ getZKManagedConsumerList ++ getHbaseManagedConsumerList, clusterContext)
  }
}
...
//
      ...
      case KSGetAllConsumerDescriptions(lastUpdateMillisOption) =>
        ...
        if (offsetCache.lastUpdateMillis > lastUpdateMillis) {
          asyncPipeToSender {
            ConsumerDescriptions(offsetCache.getConsumerList.list
