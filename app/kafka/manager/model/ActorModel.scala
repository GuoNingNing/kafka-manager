...
object ActorModel {
  ...
  // 获取指定Topic的指定时间的Offset
  //  case class KSGetOffsetForTime(broker: String, topic: String, time: Long) extends QueryRequest


  sealed trait ConsumerType

  case object ZKManagedConsumer extends ConsumerType {
    override def toString() = "ZK"
  }

  case object KafkaManagedConsumer extends ConsumerType {
    override def toString() = "KF"
  }

  case object HbaseManagedConsumer extends ConsumerType {
    override def toString() = "HB"
  }
  ...
  object ConsumerType {
    def from(s: String): Option[ConsumerType] = {
      s.toUpperCase() match {
        ...
        case "HB" => Option(HbaseManagedConsumer)
        case _ => None
      }
    }
  }
  ...
  object TopicIdentity extends Logging {
    ...
    implicit def from(brokers: Int,
      ...
      val size = tpi.flatMap(_._2.leaderSize).reduceLeftOption {
        _ + _
      }.map(FormatMetric.sizeFormat(_))
